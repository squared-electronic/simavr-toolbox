...
void hd44780_print(hd44780_t *b, std::string &s) {
  s.reserve(100);
  s += "/******************\\\n";
  const uint8_t offset[] = {0x00, 0x40, 0x00 + 20, 0x40 + 20};
  for (int i = 0; i < b->h; i++) {
    s += "|";
    s.append(b->vram + offset[i], b->vram + offset[i] + b->w);
    s += " |\n";
  }
  s += "\\******************/\n";
}
  ...
  if (hd44780_get_flag(b, HD44780_FLAG_I_D)) {  // incrementing
    if (b->cursor < 0x80) {                     // cursor in DDRAM
      ...
      if (hd44780_get_flag(b, HD44780_FLAG_N)) {  // 2-line display
        if (b->cursor >= 0x00 + 40 &&
            b->cursor <
                0x40)  // jump from end of first memory segment to the start of the second segment
          b->cursor = 0x40;
        else if (b->cursor >= 0x40 + 40)  // wrap around from the end of the second memory segment
    ...
    if (b->cursor < 0x80) {                       // cursor in DDRAM
      if (hd44780_get_flag(b, HD44780_FLAG_N)) {  // 2-line display
        if (b->cursor == 0x40)  // fall back from the start of the second memory segment to the end
                                // of the first segment
          b->cursor = 0x00 + 39;
        else if (b->cursor == 0x00)  // wrap around from the start of the first memory segment to
...
static uint32_t hd44780_write_command(hd44780_t *b) {
  ...
  switch (top) {
    ...
    case 7:  // 1 ADD ADD ADD ADD ADD ADD ADD
      ...
      if (hd44780_get_flag(b, HD44780_FLAG_N)) {  // 2-line display
        if (b->cursor >= 0x00 + 40 &&
            b->cursor < 0x40)  // illegal address after the first memory segment -> set cursor to
                               // start of second segment
          b->cursor = 0x40;
        else if (b->cursor >= 0x40 + 40)  // illegal address after the second memory segment -> set
                                          // cursor to start of first segment
          b->cursor = 0x00;
      } else {                       // 1-line display
...
static uint32_t hd44780_process_read(hd44780_t *b) {
  ...
  if (!done) {  // new read
    ...
    if (four) b->flags |= (1 << HD44780_FLAG_LOWNIBBLE);  // for next read
  }
...
static avr_cycle_count_t _hd44780_process_e_pinchange(struct avr_t *avr, avr_cycle_count_t when,
                                                      void *param) {
  hd44780_t *b = (hd44780_t *)param;
...
static void hd44780_pin_changed_hook(struct avr_irq_t *irq, uint32_t value, void *param) {
  ...
  switch (irq->irq) {
    ...
    case IRQ_HD44780_D0 ... IRQ_HD44780_D7:
      ...
      if (hd44780_get_flag(b, HD44780_FLAG_REENTRANT)) return;
      break;
  }
  ...
  if (!eo && e) avr_cycle_timer_register(b->avr, 1, _hd44780_process_e_pinchange, b);
}
...
void hd44780_free(struct hd44780_t *b) {
  avr_free_irq(b->irq, IRQ_HD44780_COUNT);
}